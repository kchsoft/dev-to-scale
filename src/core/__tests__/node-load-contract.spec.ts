...
describe('node load contract', () => {
  it('stores several normalized resources and derives independent node bottlenecks', () => {
    ...
    expect(load.resources[0]).toMatchObject({
      demand: 60,
      nominalCapacity: 100,
    ...
    expect(load.nominalLoadRatio).toBe(0.9);
    expect(load.effectiveLoadRatio).toBe(0.9);
  ...
  it('keeps empty-resource nodes at zero and rejects duplicate resource kinds', () => {
    const external = createNodeLoadSnapshot('external', 'EXTERNAL_SERVICE', []);
    expect(external.nominalLoadRatio).toBe(0);
    expect(external.effectiveLoadRatio).toBe(0);
  ...
  it('queries exact nodes and uses effective pressure with input order to break ties', () => {
    ...
    expect(resourceLoad(first, 'CPU')?.effectiveRatio).toBe(0.8);
