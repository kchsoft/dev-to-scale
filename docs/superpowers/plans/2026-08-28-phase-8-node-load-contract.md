...
Migrate the existing topology view consumer in the same atomic contract change: derive its displayed capacity from the first maximum-ratio resource (strict `>` tie handling, so normalized resource order wins ties), retaining the graph-capacity fallback when no node load exists, and rebuild its test fixtures with the canonical factories. This preserves the legacy bottleneck-capacity display rather than selecting the largest absolute capacity. This is a representation-only adaptation; preserve all rendered values, topology filtering, trace behavior, and copy.
